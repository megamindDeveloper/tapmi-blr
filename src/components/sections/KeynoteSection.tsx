  ...
  function SpeakerCard({ sp, onClick }: { sp: Speaker; onClick: () => void }) {
    return (
      <div
      className="w-full max-w-[180px] cursor-pointer my-2 flex flex-col"
      onClick={onClick}
    >
      ...
      {/* Text section aligned */}
      <div className="text-left flex flex-col justify-between flex-1">
        <p className="text-md font-semibold tracking-tight mb-2 min-h-[44px]">
          {sp.name}
        </p>

        <button
          onClick={onClick}
          className="text-[#2E8E9E] md:text-sm text-md font-normal md:font-bold tracking-tight border-b border-[#2E8E9E] leading-[24px] md:leading-[16px] hover:text-[#72bfcd] transition-all cursor-pointer w-fit"
        >
          Learn More
        </button>
      </div>
